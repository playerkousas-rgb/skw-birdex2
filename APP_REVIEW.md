...
### 其他遊戲化點子（未實作，等你挑）
1. **每日任務**：每天 3 個小任務（捕捉 1 隻 / 捕捉 3 種不同鳥 / 用變焦捕捉…），完成領 XP
2. **成就徽章牆**：首次捕捉、湊滿 10/25/50 種、第一隻 LR、色違數量、登入 30 天…每解鎖一個亮徽章
3. **抽卡包**：升級或任務送「鳥卡包」，開包 3 連抽動畫
4. **熱點探索（GPS）**：走到香港 27 個觀鳥熱點附近 → 捕捉加成 + 熱點收集圖章
5. **夥伴鳥**：選一隻鳥當夥伴，顯示在訓練師頁，累積捕捉數培養親密度
6. **照片牆**：把真實捕捉的照片做成「我的捕捉紀錄」畫廊（照片目前有存但沒顯示）
7. **稱號更多元 / 每週挑戰 / 排行榜**（需要後端）
